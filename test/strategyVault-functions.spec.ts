...
describe('strategy-vault-test', () => {
    ...
    before(async() => {
        ({sGlp, esSPY, feeSpyGlpTracker, stakedSpyGlpTracker, batchRouter, strategyVault, router, gmxHelper} = await sympathyProtocolFixture());
        ...
        await batchRouter.connect(whale).claimStakedSpyGlp();
    })
    ...
    it('checks exitStrategy function', async() => {
        ...
        let balance = await stakedSpyGlpTracker.balanceOf(whale.address);
        await router.connect(whale).settle(balance);
        expect(await sGlp.totalSupply()).eq(0);
    })
})
