...
contract IDO is ReentrancyGuard, Governable {
    ...
    mapping(uint256 => bool) public isClaimPublicSale;
    ...
    function setClaimStatus(bool _isClaimWhitelistSale, bool _isClaimRef) external onlyGov {
      isClaimWhitelistSale = _isClaimWhitelistSale;
      isClaimRef = _isClaimRef;
    }

    function setClaimPublicSaleStatus(uint256 _round, bool _status) external onlyGov {
      isClaimPublicSale[_round] = _status;
    }
    ...
    function claimToken(uint256 _round) external nonReentrant {
      require(isClaimPublicSale[_round], "IDO: claim token not active");
