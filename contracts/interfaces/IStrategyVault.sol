...
interface IStrategyVault {
    ...
    function buySpyGlp(uint256 _amountIn) external returns (uint256);

    function sellSpyGlp(uint256 _glpAmount, address _recipient) external returns (uint256);
    
    function reExecuteBatchPositions() external returns (uint256);
