...
contract StrategyVault is Initializable, UUPSUpgradeable {
    ...
    uint256 public pendingIncreasePositionAmount;
    ...
    event BuySpyGlp(uint256 amountIn, uint256 amountOut, uint256 value);
    event SellSpyGlp(uint256 amountIn, uint256 amountOut, address recipient);
    ...
    function executeIncreasePositions(bytes[] calldata _params) external payable onlyRouter {
        ...
        for (uint256 i=0; i<2; i++) {
            ...
            pendingIncreasePositionAmount += amountIn;
    ...
    function buySpyGlp(uint256 _amountIn) external onlyRouter returns (uint256) {
        require(confirmed, "StrategyVault: not confirmed yet");
        IGmxHelper _gmxHelper = IGmxHelper(gmxHelper);
        ...
        pendingShortValue = 0;
        pendingIncreasePositionAmount = 0;
        
        emit BuySpyGlp(_amountIn, amountOut, value);

        return value;
    }

    function sellSpyGlp(uint256 _glpAmount, address _recipient) external onlyRouter returns (uint256) {
        require(confirmed, "StrategyVault: not confirmed yet");

        uint256 amountOut = sellGlp(_glpAmount, _recipient); 
  
        emit SellSpyGlp(_glpAmount, amountOut, _recipient);
    ...
    function _collectManagementFee() internal {
        ...
        uint256 supply = IERC20(sGlp).totalSupply() - IERC20(sGlp).balanceOf(management);
        uint256 alpha = supply * MAX_BPS / (MAX_BPS - (managementFee * duration / SECS_PER_YEAR)) - supply;
        if (alpha == 0) {
            return;
        }
        IMintable(sGlp).mint(management, alpha);
    ...
    function settle(uint256 _amount, address _recipient) external onlyRouter {
        ...
        uint256 supply = IERC20(sGlp).totalSupply();
    ...
    function reExecuteBatchPositions() public onlyRouter returns (uint256) {
        require(pendingIncreasePositionAmount > 0, "StrategyVault: pendingIncreasePositionAmount > 0");

        uint256 amount = pendingIncreasePositionAmount;
        IERC20(want).transfer(address(msg.sender), amount);
